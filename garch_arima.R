install.packages(c("quantmod", "lattice", "timeSeries", "rugarch"))
...
for (d in 0:foreLength){
  ...
  # obtain the S&P 500 rolling window for this day
  
  spReturnsOffset = spReturns[(1+d):(windowLength)]
...
# Output the CSV file to "forecast.csv"
write.csv(forecasts, file="forecast.csv", row.names = FALSE)

# Input the Python-refined CSV file AFTER CONVERSION
spArimaGarch = as.xts(
  read.zoo(
    file="forecast_new.csv", format="%Y-%m-%d", header=F, sep="," 
  )
)

# Create the ARIMA+GARCH returns
spIntersect = merge( apArimaGarch[,1], spReturns, all=F )
spArimaGarchReturns = spIntersect[,1] * spIntersect[,2]

# Create the backtest for ARIMA+GARCH and Buy & Hold
